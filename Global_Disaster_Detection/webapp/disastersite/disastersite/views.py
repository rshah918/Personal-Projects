...
def get_match(disaster):

    def get_top_locations(grouped_by_location, cleanedTweets):
        top_location_indices = []
        top_locations = []
        while ((len(top_location_indices) < 5) and (len(top_locations) <= len(grouped_by_location))):
            max_count = 0
            max_index = 0
...
def indexView(request):
    return render(request, 'disastersite/index.html')

def myView(request):
    disaster = request.POST.get("disaster")
    match = get_match(disaster)
